document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchTopPosts() {
        try {
            const response = await fetch("events.php?action=get_top_posts");
            if (!response.ok) {
    ...
    document.getElementById("dateFilter").addEventListener("change", async (event) => {
        ...
        try {
            ...
            const response = await fetch(`events.php?action=filter_posts&order=${order}`);
            if (!response.ok) {
    ...
    document.getElementById("searchButton").addEventListener("click", async () => {
        ...
        try {
            ...
            const response = await fetch(`events.php?action=search_posts&query=${encodeURIComponent(query)}`);
            if (!response.ok) {
